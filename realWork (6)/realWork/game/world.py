...
from .buildings import Castle, House, Mining, Wood
from .animation import Build, MySprite
...
class World:
    ...
    def update(self, camera):
        ...
        if (keys[pg.K_RIGHT]):
            self.p.run_right()
        # if player moves to upward
        if (keys[pg.K_UP]):
            self.p.run_up()
        # if player moves to the downward
        if (keys[pg.K_DOWN]):
            self.p.run_down()
        if mouse_action[2]:
            self.examine_tile = None
        ...
        if self.hud.selected_tile is not None:
            ...
            if self.can_place_tile(grid_pos):
                ...
                if mouse_action[0] and not collision:
                    if self.hud.selected_tile["name"] == "Castle":
                        ent = Castle(render_pos, self.resource_manager)
                    ...
                    elif self.hud.selected_tile["name"] == "Mining":
                        ent = Mining(render_pos, self.resource_manager)
                        self.entities.append(ent)
                        self.buildings[grid_pos[0]][grid_pos[1]] = ent
                    elif self.hud.selected_tile["name"] == "Wood":
                        ent = Wood(render_pos, self.resource_manager)
                        self.entities.append(ent)
                        self.buildings[grid_pos[0]][grid_pos[1]] = ent
        ...
        for x in range(self.grid_length_x):
            for y in range(self.grid_length_y):
                ...
                if worker is not None:
                    ...
                    px = 600
                    py = 400
                    screen.blit(self.p.images[self.p.index], (px, py))
    ...
    def grid_to_world(self, grid_x, grid_y):
        ...
        perlin = 35 * noise.pnoise2(grid_x/self.perlin_scale, grid_y/self.perlin_scale)
        perlin2 = 35 * noise.pnoise2(grid_x / self.perlin_scale, grid_y / self.perlin_scale)
    ...
    def load_images(self):
        ...
        house = pg.image.load("assets/graphics/dar.png").convert_alpha()

        mining = pg.image.load("assets/graphics/mining.png").convert_alpha()
        wood = pg.image.load("assets/graphics/wood.png").convert_alpha()
        tree = pg.image.load("assets/graphics/tree.png").convert_alpha()
        water = pg.image.load("assets/graphics/water.gif").convert_alpha()
